...
// PNG. For Baker the active PNG is hard-pinned to the teal asset
// below; for unbranded VNDRLY (added 2026-05-08 follow-up) the active
// PNG is resolved via `pickPillForBrand(brand.primary)` so the default
// VNDRLY gold (#e6ac00) lights up amber. Partner/vendor-branded
// experiences keep the original `TintedPillBg` treatment for now.
import bakerNavTeal from "@assets/900x229_baker_teal_Pill_1778229624365.png";
...
// PILL-shape Baker hard-pin (added for the tickets-page toolbar). When
...
import bakerNavTealPill from "@assets/900x229_baker_teal_Pill_1778246521449.png";
 ...
 * Used by the Sign Out button so it lights up like the active nav item.
 */
export default function SidebarButton({
  ...
  /**
   * Optional override for the active/hover layer PNG. When provided, this
   ...
   * — Baker always uses its hard-pinned asset for `shape`.
   */
  activeSrcOverride?: string;
  /**
   * Visual shape of the active-layer asset. `"square"` (default) uses the
   * square Baker hard-pin and the square palette — matches the original
   * sidebar nav buttons. `"pill"` uses the rounded-end Baker hard-pin and
   * the pill palette for non-Baker brands — used by the tickets-page
   * toolbar buttons (Start New, Phone intake, Group by visit). Honors the
   * pill-for-pill / square-for-square substitution doctrine.
   */
  shape?: "square" | "pill";
  className?: string;
  idleOpacityClass?: string;
  /**
  ...
  // VNDRLY (default + admin-set platform brand color) — uses the
  ...
  // re-picked automatically on the next render (no manual asset
  // swap needed). Baker keeps its hard-pinned teal asset because
  // its brand color (#00ADB5) maps to `bakerTeal` in the palette
  // anyway and the dedicated PNG is hand-tuned for that surface.
  const useBakerStyle = true;
  // Square-for-square: the Baker hard-pinned asset is visually a square
  // (despite "Pill" in its filename), so the non-Baker substitute must
  // also come from the square palette (default shape).
  // Baker doctrine (replit.md): Baker ALWAYS uses its hard-pinned teal asset
  // for the active/hover layer — no per-button overrides. Only non-Baker
  ...
  const activePillSrc = activeSrcOverride
    ...
    : isBaker
      ...
      : pickPillForBrand(brand.primary, shape);
  ...
  if (useBakerStyle) {
    ...
    //     explicitly asked for that same "previous buttons" rest
    ...
    const greyClass = isActive
      ? "opacity-0"
      : `${idleOpacityClass} group-hover:opacity-0`;
  ...
  return (
    ...
    >
      ...
      {/* Active gloss — white silhouette at 50% on the top half only, with
          the bottom half clipped so the brand color shines through fully. */}
      <TintedPillBg
        src={btnGrey}
        color="#ffffff"
        className={cn(
          "[clip-path:inset(0_0_50%_0)] transition-opacity duration-200",
          whiteGlossClass,
        )}
      />
