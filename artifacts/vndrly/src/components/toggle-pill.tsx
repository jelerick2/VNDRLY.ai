...
// `TogglePillButton` swaps the brand-matched hover/pulse PNG (which
...
// because `TogglePillButton` is pill-shaped at rest. The other Baker
...
import bakerPillTeal from "@assets/900x229_Amber_Pill2_-_Copy_1778229329456.png";
import primaryHoverPill from "@assets/900x229_Blue_Pill_1777099484818.png";
...
import semanticHoverGreen from "@assets/900x229_Green_Pill_1777099484825.png";
import semanticHoverAmber from "@assets/900x229_Amber_Pill_v4_1778504507024.png";

const BRAND_PILL_ASPECT = 900 / 229;

/**
 * # TogglePill — the canonical VNDRLY pill visual language
 *
 ...
 *
 * ## DOCTRINE — when "TogglePill" is asked for, USE THIS FILE
 *
 * The user has been emphatic: "TogglePill" means **exactly** the
 * components exported from this file — `<TogglePill>` for read-only
 * chips, `<TogglePillButton>` for interactive buttons. Do not
 ...
 *
 * 1. **Colored** (default for `<TogglePill>`, hover/pulse state of
 *    `<TogglePillButton>`) — the active/colored half of the EN/ES
 ...
 *    against any tonal fill. Pass `color` to pick the tone.
 * 2. **Rest** (`rest` prop on `<TogglePill>`, idle state of
 *    `<TogglePillButton>`) — the shared PNG image-asset chrome of
 ...
 * MUST pick the color by semantic, not by aesthetic — the whole
 * point of TogglePill is that "green chip" reads the same wherever
...
export const TOGGLE_PILL_GLOSS_GRADIENT =
...
export const TOGGLE_PILL_TEXT_SHADOW = "0 2px 4px rgba(0,0,0,0.9)";

/** Canonical color palette. Keys are the public `color` API. */
...
export type TogglePillColor = keyof typeof TOGGLE_PILL_COLORS;
...
/**
 * Read-only pill chip rendered in the canonical TogglePill visual
...
export default function TogglePill({
  ...
  if (rest) {
    return (
      ...
      >
        ...
        <span
          className={cn(
            "relative z-10 flex items-center justify-center w-full h-full font-bold whitespace-nowrap text-gray-700",
            labelClass,
          )}
        >
          {children}
  ...
  // Canonical pill-doctrine: every colored TogglePill renders the
  // matching canonical color PNG (no CSS solid fills). `brand` resolves
  // to the canonical blue pill since the new palette is brand-agnostic.
  const colorPng =
    color === "red"
      ? semanticHoverRed
      : color === "green"
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{ textShadow: TOGGLE_PILL_TEXT_SHADOW }}
      >
        {children}
      </span>
    </div>
...
interface TogglePillButtonProps {
  ...
  attention?: boolean;
  className?: string;
  /** Native `title` tooltip on the underlying `<button>`. */
  title?: string;
  "data-testid"?: string;
}

/**
 * Interactive sibling of `TogglePill` — a button that BEHAVES like
 * the EN/ES `LanguageToggle`: a clean rounded-full chip that's
 * white at rest with grey text, then transitions to the canonical
 * colored TogglePill (solid tonal fill + 50% white top-half gloss
...
export function TogglePillButton({
  ...
  // animation (700 ms toggle). When pulsing, the chip alternates
  // between its idle (white/grey) state and its colored TogglePill
  ...
  // tonal fill so generic deployments still get the TogglePill palette.
  ...
  //   - color="red"/"green"/"amber": the corresponding palette PNG
  ...
  return (
    <button
      ...
      data-testid={props["data-testid"]}
    >
      {/* Layer 0 — light-grey pill PNG, shared rest chrome of the
          whole pill family. 90% at rest, FADES OUT to 0 on
          hover/pulse so the colored hover PNG above renders cleanly
          without grey bleeding through the anti-aliased end-cap
          edges (the cap-shading artifact the user flagged on the
          Schedule button). */}
      <PillBg
        src={pillBase}
        className={cn(
      ...
      <span
        className={cn(
          ...
          pulseOn
            ...
            : "text-gray-800/85 group-hover:text-white group-hover:drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]",
        )}
      >
        {children}
      </span>
