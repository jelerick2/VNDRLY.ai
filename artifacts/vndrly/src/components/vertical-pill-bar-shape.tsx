...
import { statusToPillColor } from "@/components/pill-bar-shape";
...
import statusPillAmber from "@assets/900x229_Amber_Pill_1778247577557.png";
import statusPillBlue from "@assets/900x229_blue_Pill_v4_1778247577557.png";
import statusPillGreen from "@assets/900x229_green_Pill_v4_1778247577559.png";
import statusPillRed from "@assets/900x229_dark_red_Pill_v2_1778247577558.png";
import statusPillLightGrey from "@assets/900x229_Light-grey_v2r_Pill_1778247577555.png";
// Status-specific override art that doesn't fit the standard
// `PillColor` palette. `pending_review` uses a brighter amber-v4
// pill so the in-progress-but-blocked-on-office bar reads as a
// distinct "needs attention" tone rather than the muted grey it
// inherits from the badge color bucket.
import statusPillAmberV4 from "@assets/900x229_Amber_Pill_v4_1778274328575.png";

const PILL_IMG_SRC: Record<PillColor, { src: string; light?: boolean }> = {
  amber: { src: statusPillAmber },
  blue: { src: statusPillBlue },
  green: { src: statusPillGreen },
  red: { src: statusPillRed },
  grey: { src: statusPillLightGrey, light: true },
};

// Per-status image override. Takes precedence over the color-based
// lookup in `PILL_IMG_SRC` so a single status can opt into bespoke
// art without polluting the shared `PillColor` palette. Keys are raw
// ticket status strings (matches `payload.status` from the chart).
const STATUS_IMG_OVERRIDE: Record<string, { src: string; light?: boolean }> = {
  pending_review: { src: statusPillAmberV4 },
};
...
interface VerticalPillBarShapeProps {
  ...
  width?: number;
  height?: number;
  payload?: { color?: PillColor; status?: string };
...
export function VerticalPillBarShape(props: VerticalPillBarShapeProps) {
  ...
  const color: PillColor =
    payload?.color ?? (status ? statusToPillColor[status] : undefined) ?? fallbackColor;
  const pill = STATUS_IMG_OVERRIDE[status] ?? PILL_IMG_SRC[color];
  ...
  return (
    ...
    >
      ...
      <g
        clipPath={`url(#${glossId})`}
        opacity={pill.light ? 0.5 : undefined}
      >
