...
import toolbarPillActiveSrc from "@assets/900x229_baker_teal_Pill_1778246521449.png";
import bakerNavTealSquareSrc from "@assets/900x229_baker_teal_Pill_1778229624365.png";
import bakerTealPillRoundSrc from "@assets/900x229_baker_1_1778258865032.png";
import userBubblePillSrc from "@assets/900x229_baker_1_1778259114836.png";
 ...
 * from, so this is the only signal we have for picking up
...
export interface AssistantPanelProps {
   ...
   * persona-scoped public chat endpoint, hides controls that imply
...
// visitor has obvious starter prompts. Kept short and concrete: each
...
// baker PNG with the same vibrant filter; default brands composite the
...
// pill) with the icon overlaid; bumps to 100% on hover.
...
export function AssistantPanel({ open, onOpenChange, tokenMode, signupMode }: AssistantPanelProps) {
  ...
  // open so a return visit picks up where they left off. The hook is
  ...
  // Effect deps are intentionally just `open` so messages.length /
    ...
    fetch(url, { credentials: "include" })
      ...
      .then((data) => {
        ...
        if (tokenMode) {
          ...
          if (p && p.currentStep && p.currentStep !== "done") {
            setProgress({
              orgType: "field_employee",
              currentStep: p.currentStep,
              completedSteps: Array.isArray(p.completedSteps) ? p.completedSteps : [],
              skippedSteps: Array.isArray(p.skippedSteps) ? p.skippedSteps : [],
            });
          } else {
            setProgress(null);
          }
        ...
        const p = data?.progress;
        if (p && (p.orgType === "partner" || p.orgType === "vendor") && p.currentStep && p.currentStep !== "done") {
          setProgress({
            orgType: p.orgType,
            currentStep: p.currentStep,
            completedSteps: Array.isArray(p.completedSteps) ? p.completedSteps : [],
  ...
  // Accept the offered pre-auth chat: ask the server to spin up a new
  // conversation row seeded with the visitor's prior turns, then hide
  // the banner. On failure we leave the banner up so the user can try
  // again — the only failure modes are a transient network blip or a
  // 401, both of which a retry will fix.
  const handleAcceptPendingSignup = async () => {
  ...
  // pre-load the prior context into the model.
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        bare
        className="sm:max-w-lg h-[min(80vh,640px)] bg-[#3a3d42] text-gray-100 border-white/20"
        data-testid="assistant-panel"
        hideClose
      >
        <DialogHeader
          className={cn(
            "relative z-10 shrink-0 border-b border-white/20 bg-transparent px-3 py-0 flex-row items-center justify-between space-y-0 pr-3",
            // No vertical padding — header height = tallest child only, so
            // the strip collapses to exactly the space its content needs.
          )}
        >
          ...
          <div className="flex items-center gap-1">
            {/* Pre-auth EN/ES toggle, only visible on the public
                ...
                preference yet, so we let them flip explicitly when
            ...
            {/* Header action buttons only render once a conversation is in
                ...
                lets the header collapse to its minimum height. */}
            {messages.length > 0 && (
              <>
                <PillButton
                  color="image"
            ...
            {/* Inline close button — kept in the same flex row as the
                ...
                `top-4` and would sit below the header center on this
                py-0 header. */}
            <PillButton
              color="red"
        ...
        {pendingSignup && (
          <PendingSignupChatOffer
            chat={pendingSignup}
            adopting={adopting}
            onAccept={handleAcceptPendingSignup}
            onDecline={handleDeclinePendingSignup}
          />
        )}

        <div ref={scrollRef} className="relative z-10 min-h-0 flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {messages.length === 0 && (
              ...
              {quickActions.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {quickActions.map((qa) => (
                    ...
                    >
                      <PillBg
                        src={isBaker ? bakerTealPillRoundSrc : pickPillForBrand(brand.primary, "pill")}
                        imageAspect={900 / 229}
          ...
          {messages.map((m) => (
            ...
            >
              ...
              >
                {m.role === "user" && (
                  <PillBg
                    src={isBaker ? userBubblePillSrc : pickPillForBrand(brand.primary, "pill")}
                    imageAspect={900 / 229}
                  />
                )}
                {m.role === "assistant" ? (
          ...
          {activeTool && (
            <div className="text-xs text-muted-foreground italic px-1">
              Looking up {prettyTool(activeTool)}…
            </div>
          )}
...
// user signs in / signs up; dismissed (by accept or decline) it never
...
function PendingSignupChatOffer({
  ...
  // Pull the most recent user message as a preview so the offer is
  // recognisable ("oh right, I asked about insurance") without
  // needing the full transcript.
  const lastUser = [...chat.messages].reverse().find((m) => m.role === "user");
  const preview = lastUser?.content.trim().split(/\n+/)[0] ?? "";
  const turnCount = chat.messages.length;
  return (
    <div
      className="border-b bg-primary/5 px-4 py-3 space-y-2"
      data-testid="assistant-pending-signup-offer"
    >
      <div className="text-sm font-medium">Continue your earlier chat?</div>
      <div className="text-xs text-muted-foreground">
        We saved the {turnCount}-message conversation you started before signing
        in. Pick it back up so you don't have to re-explain.
      </div>
      {preview && (
        <div className="rounded-md bg-background border px-2 py-1.5 text-xs text-muted-foreground line-clamp-2">
          “{preview}”
        </div>
      )}
      <div className="flex items-center gap-2 pt-1">
        <PillButton
          ...
          onClick={onAccept}
          disabled={adopting}
          data-testid="assistant-pending-accept"
        >
...
// Compact horizontal stepper shown above the message list when an
// onboarding flow is in progress. Mirrors the dot/check pattern from
...
function OnboardingMiniStepper({ progress }: { progress: OnboardingProgress }) {
  ...
  return (
    <div
      className="border-b bg-muted/20 px-4 py-2 space-y-1.5"
          ...
          return (
            ...
            >
              ...
              {i < steps.length - 1 && (
                <div
                  className={
                    "h-px flex-1 " + (isDone ? "bg-primary/60" : "bg-muted-foreground/20")
                  }
                />
              )}
            </div>
...
// user can save, paste into a ticket, or share with their account
...
// Map raw tool names from the server to a friendly verb the user reads
// in the "Looking up …" indicator.
...
export function AssistantLauncher({
  ...
  const { primary } = brand;
  const isBaker = !!brand.name?.toLowerCase().includes("baker");
  // While the modal is open the launcher should sit in its plain
  // inactive state — no active/hover snap, no breathing — so it
      ...
      >
        ...
        {isBaker ? (
          <>
            ...
            />
            {/* Hidden preload of legacy assets so existing bundle
          ...
          <>
            {/* Default (non-Baker) — 4 layers. Bottom-to-top:
                  1. askVinactive @ 80%.
