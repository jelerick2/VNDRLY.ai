          ...
          <div className={`border-2 rounded-xl p-6 shadow-xl transition-colors duration-300 ${formReady ? "border-amber-500" : "border-gray-300"}`}>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input required value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="e.g. Precision Drilling" className="h-10" data-testid="signup-input-name" />
                {matches.length > 0 && (
                  <div
                    role="alert"
                ...
                {nameCheckUnavailable && (
                  <p className="mt-1 text-xs text-amber-700" role="status" data-testid="vendor-signup-match-unavailable">
