...
export default function SignupPartner() {
  ...
  // Fuzzy duplicate check via the public /partners/check-name endpoint.
  // Mirrors the vendor self-signup flow (artifacts/vndrly-mobile/app/
  // signup-vendor.tsx) so a partner whose company is already in the
    ...
    const handle = setTimeout(async () => {
      try {
        const res = await apiFetch(
          `/api/partners/check-name?name=${encodeURIComponent(trimmed)}`,
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const err: Error & { data?: unknown; status?: number } = new Error(
          data.message || data.error || "Failed to create partner",
        );
        err.data = data;
        err.status = res.status;
        throw err;
      }
      toast({ title: "Partner account created! You can now sign in." });
      navigate("/");
    } catch (err: unknown) {
      toast({
        title: translateApiError(err, t, t("errors.onboarding.create_partner_failed")),
        variant: "destructive",
      });
    }
    setSaving(false);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                {matches.length > 0 && (
                  <div
                    role="alert"
                    data-testid="partner-signup-duplicate-warning"
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 space-y-1.5">
                        ...
                        <label className="mt-1 flex items-center gap-2 text-amber-900">
                          <Checkbox
                            data-testid="partner-signup-confirm-different"
                ...
                {matchesLoading && matches.length === 0 && trimmedName.length >= 3 && (
                  <p className="mt-1 text-xs text-muted-foreground" data-testid="partner-signup-match-loading">
      ...
      <div className="hidden lg:flex flex-1 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-end justify-center p-12 w-full h-full">
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 max-w-md border-2 border-amber-500">
            <h2 className="text-xl font-bold text-white mb-2">Partner Benefits</h2>
            <p className="text-sm text-white/85 leading-relaxed">
              Oversee your drilling sites, approve vendor work orders, and maintain full visibility of field operations with real-time tracking and reporting.
            </p>
          </div>
        </div>
      </div>
