...
//     - the abstract badge color bucket (amber/blue/green/red/grey),
...
export interface TicketStatusMeta {
  // `null` means "render as plain text without a colored pill" — used
  // for transient pre-work statuses like `draft` / `initiated` that
  // shouldn't shout for attention.
...
const ticketLifecycleMeta: Record<TicketStatus, TicketStatusMeta> = {
  ...
  // shared status badge. `awaiting_acceptance` reuses the amber
  // "blocked-on-someone-else" treatment from `awaiting_payment` so
  // list-view rows surface the pending invite at a glance; `denied`
  // stays muted so a rejected invite doesn't shout for attention.
  awaiting_acceptance: {
    badgeColor: "amber",
