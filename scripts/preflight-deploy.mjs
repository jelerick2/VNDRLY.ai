...
console.log("SUPABASE_URL:", ok(localEnv.supabase_url || supabaseEnv.supabase_url));
console.log("SUPABASE_SERVER_KEY:", serverKey && !/YOUR_/i.test(serverKey) ? "OK" : "MISSING (uploads use disk on server without this)");
...
const blockers = [];
if (!serverKey || /YOUR_/i.test(serverKey)) {
  blockers.push("- SUPABASE_SECRET_KEY (preferred) or SUPABASE_SERVICE_ROLE_KEY in Supabase.env");
}
if (!realVps) {
